...
[NOTE]
====
При установке из tar-архива сервер будет установлен в директорию [path]_/opt/firebird_

В отличие от установки из репозиториев утилита [app]``isql`` не переименована и расположена в [path]_/opt/firebird/bin_.
====
...
Особенно важно убедиться, что файлы [path]_fbclient.dll_ и [path]_gds32.dll_ удалены из каталога [path]_system32_.
...
Позже вы можете изменить архитектуру через файл конфигурации [path]_firebird.conf_ (параметр ServerMode), что потребует перезагрузки, но не переустановки.
...
Тем не менее, данная возможность доступна опционально в процессе установки, так же как и копирование [path]_fbclient.dll_ в системные папки Windows.
...
Это можно сделать двумя способами: с использованием консольного инструмента для выполнения интерактивных запросов [app]``isql.exe`` и консольного инструмента для управления базой данных безопасности [app]``gsec.exe``.
...
Запустите инструмент для выполнения интерактивных запросов [app]``isql.exe``.
...
Запустите [app]``gsec.exe`` указав пользователя SYSDBA и базу данных [path]_security.db_.
...
[NOTE]
====
Инструмент [app]``gsec.exe`` является устаревшим, многие возможности доступные через SQL не доступны в нём.
====
...
Если вы хотите чтобы сервер запускался в другой архитектуре, то необходимо изменить значение параметра [parameter]``ServerMode`` в [path]_firebird.conf_.
...
[NOTE]
====
Если вам не нужна поддержка безопасной парольной аутентификации (SRP), удалите из [parameter]``AuthServer`` плагины Srp256 и Srp; из [parameter]``AuthServer`` -- Legacy_UserManager, а [parameter]``WireCrypt`` можете изменить на Disabled.
====
...
Для одновременного запуск нескольких Firebird, необходимо развести их по разным портам tcp (если конечно слушатель запущен в режиме прослушивания TCP/IP). Для этого необходимо изменить в [path]_firebird.conf_ параметр [parameter]``RemoteServicePort``.
...
Если необходимо обеспечить также одновременную работоспособность по локальному протоколу XNET и через именованные каналы WNET, то необходимо так же изменить параметры [parameter]``IpcName`` и [parameter]``RemotePipeName``.
Однако стоит учесть, что эти параметры придётся изменять и на стороне клиента через DPB.

[IMPORTANT]
====
Если вы хотите запускать несколько экземпляров Firebird, то не используйте утилиту [app]``instreg.exe``, которая записывает путь к серверу в реестр Windows.
====

[[fbadmdg-install-windows_instreg]]
==== Запись в реестре

Утилита [app]``instreg.exe`` позволяет прописывать в реестр Windows путь к корневому каталогу сервера.
...
[IMPORTANT]
====
Если вы хотите запускать несколько экземпляров Firebird, то не используйте утилиту [app]``instreg.exe``.
====
...
Утилита [app]``instsvc.exe`` записывает, удаляет или меняет информацию о запуске сервера в базе сервисов операционной системы.
...
Для упрощения процедуру установки и удаления служб в ZIP архиве в комплекте с Firebird поставляются два BAT файла: [path]fbadmdg-install-service.bat_ и [path]_uninstall_service.bat_.
...
Если речь идёт об установке только о клиентской части, то обязательно требуется файл [path]_fbclient.dll_.
Клиент Firebird 3.0 обязательно требует наличие установленного Microsoft Runtime C++ 2010 соответствующей разрядности.
Если данная библиотека не установлено, то можно скопировать дополнительные библиотеки, которые поставляются в ZIP архиве под Windows [path]_msvcp100.dll_ и [path]_msvcr100.dll_.

Желательно, чтобы рядом с [path]_fbclient.dll_ был расположен файл сообщений [path]_firebird.msg_.
Большинство сообщений об ошибках уже содержатся в [path]_fbclient.dll_, однако если вы собираетесь пользоваться консольными утилитами файл [path]_firebird.msg_ обязательно должен присутствовать.

Если требуется сжатие трафика при работе по TCP/IP, то потребуется так же библиотека [path]_zlib1.dll_.

Для того чтобы клиентское приложение могло загрузить библиотеку [path]_fbclient.dll_ она должна располагаться либо рядом с приложением, либо в одной из директорий в которой производится поиск, например добавленной в [path]_PATH_ или системной директории для размещения общедоступных библиотек ([path]_system32_ или [path]_SysWOW64_).

[IMPORTANT]
====
Размещение клиентской библиотеки в [path]_PATH_ может помешать другим приложениям, которым требуется клиентская библиотека другой версии или другого сервера.
Поэтому, если предполагается, что приложение должно работать независимо от других приложений с конкретной версией клиента, то файлы клиента требуется разместить в папке приложения, и не прописывать этот путь в [path]_PATH_.
====
...
Утилита [app]``instclient.exe`` позволяет:

* установить клиентскую часть одной командой;
* установить клиентскую часть как [path]_fbclient.dll_ и/или [path]_gds32.dll_;
* проверить наличие установленной библиотеки [path]_fbclient.dll_ и/или [path]_gds32.dll_;
* удалить ранее установленные [path]_fbclient.dll_ и/или [path]_gds32.dll_.


[NOTE]
====
Утилита [app]``instclient.exe`` должна быть запущена с правами администратора.
====

Если запустить [app]``instclient.exe`` без параметров, то будет выведена краткая справка:
...
Некоторые старые приложения требуют для работы клиентскую библиотеку [path]_gds32.dll_ (наследие Interbase). Кроме того, многие приложения требуют для работы не просто [path]_gds32.dll_, но и проверяют версию этой библиотеки.
Именно поэтому простое переименовывание [path]_fbclient.dll_ в [path]_gds32.dll_ для таких программ работать не будет -- версия библиотеки окажется ниже 6.0 (т.к.
соответствует версии Firebird, у которого нумерация версий идет с 1.0). Утилита [app]``instclient.exe`` позволяет не просто установить [path]_gds32.dll_, но и изменить её версию на корректную.
...
Утилита [app]``instclient.exe`` позволяет также проверить наличие установленной клиентской библиотеки с помощью команды
...
[NOTE]
====
Обычно обновление в рамках пойнт-релизов не требует процедуры резервного копирования и восстановления базы данных средством [app]``gbak``, но бывают исключения.
...
. Заменить новые файлы конфигурации ([path]_firebird.conf_, [path]_databases.conf_ и другие), а также файл [path]_security3.fdb_ на прежние
...
* клиентская библиотека ([path]_fbclient.dll_ или [path]_libfbclient.so_);
* движок для работы с ODS 12.0 ([path]_engine12.dll_ или [path]_libEngine12.so_) из папки [path]_plugins_;
* библиотека интернационализации, которая находится в каталоге [path]_intl_; 
* файл сообщений [path]_firebird.msg_;
* для изменения некоторых параметров могут потребоваться [path]_firebird.conf_ и/или [path]_databases.conf_;
* в Windows обязательно необходимы файлы ICU библиотеки;
* если в Windows не установлен MS VC++ Runtime 2010, то необходимы также файлы ([path]_msvcp100.dll_ и [path]_msvcr100.dll_);
* если вам необходимы консольные инструменты администрирования, то скопируйте также [path]_gbak_, [path]_gfix_, [path]_gstat_, [path]_isql_, [path]_nbackup_.
...
Для изменения режима запуска необходимо отредактировать (возможно раскомментировать) значение параметра [parameter]``ServerMode`` на Classic или SuperClassic в файле [path]_firebird.conf_.

Поскольку в embedded варианте сетевой доступ не используется вы можете отредактировать также провайдеры [parameter]``Providers``, удалив от туда лишние провайдеры Remote и Loopback.
